...
export class ApiHelperService {
  ...
  getRegionData(region: string): Observable<string[]> {
    if (region.trim() === '') {
      return of([]);
    }
    ...
    for (const value of Object.values(this.countryData)) {
      if (value.region.toLowerCase().includes(region.toLowerCase())) {
        if (!reqData.includes(value.region)) {
          reqData.push(value.region);
        }
      }
    }
    return of(reqData.slice(0, 10));
  }

  getCountryData(country: string, region: string): Observable<string[]> {
    if (country.trim() === '') return of([]);
    const reqData: string[] = [];
    for (const value of Object.values(this.countryData)) {
      if (
        value.country.toLowerCase().includes(country.toLowerCase()) &&
  ...
  saveCustomerData(customer: customer) {
    let customers = localStorage.getItem("customersData");
    let newCustomers: customer[] = [];
    if (customers) {
      newCustomers = [...JSON.parse(customers) as customer[]];
    }
    newCustomers.push(customer);
    this.customer$.next(newCustomers)
    localStorage.setItem("customersData", JSON.stringify(newCustomers));
  }
